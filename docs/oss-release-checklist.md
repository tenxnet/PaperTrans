...
This checklist tracks the remaining work for the PaperTrans v0.2.0-rc.1 source release.
...
- [x] Document official arXiv HTML plus MCP translation as the supported v1 path; mark Docling PDF import experimental.
...
- [ ] Verify the one-command setup and startup flow from a clean macOS checkout and a clean Linux checkout.
- [ ] Verify first-run Docling model preparation and a subsequent network-free restart.
- [ ] Create a `v0.2.0-rc.1` tag and release notes with the known limitations from `CHANGELOG.md`.
- [ ] Confirm the source distribution notice for the AGPL/commercial PyMuPDF dependency and any separate Docling model licenses.

## Recommended for the first stable tagged release
...
- [ ] Before distributing application binaries, decide whether to retain, replace, or further isolate the AGPL/commercial PyMuPDF dependency used by experimental PDF and PDF-figure conversion paths.
...
- Signed desktop packaging, automatic updates, and removal of the source-checkout runtime prerequisites.
