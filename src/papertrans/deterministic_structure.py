...
def _body_font(pages: list[dict[str, Any]]) -> float:
    ...
    if likely_body:
        return float(median(likely_body))
    all_fonts = [
        float(block.get("fontSizeMax", 0))
        for page in pages
        for block in page["blocks"]
...
def _fallback_visual_rect(
    ...
    for block in page["blocks"]:
        ...
        small = float(block.get("fontSizeMax", body_font)) <= body_font * 0.76
        ...
        if kind == "table" and -0.02 <= rect[1] - caption[3] <= 0.35:
            ...
            nearby.append(rect)
    return _union(nearby) if len(nearby) >= 2 else None
...
def _looks_like_equation(block: dict[str, Any], body_font: float) -> bool:
    text = str(block["text"]).strip()
    if not text or len(text) > 260 or CAPTION.match(text):
        return False
    rect = _rect(block["bboxNormalized"])
    centered = rect[0] > 0.16 and rect[2] < 0.84
    ...
    return centered and has_operator and (equation_number or symbol_count >= 3) and word_count <= 8 and float(
        block.get("fontSizeMax", body_font)
    ) <= body_font * 1.15
    ...
    for page in pages:
        ...
        first_heading_y = min(
            (
                ...
                for block in page["blocks"]
                if _heading(block, body_font) is not None and block["blockId"] not in visual_block_ids
            ),
            default=1.0,
        )
        seen_title = False
            ...
            if block_id in visual_block_ids:
                ...
                if PAGE_NUMBER.match(text) and (rect[1] > 0.9 or rect[3] < 0.1):
                    ...
                    if heading is not None:
                        ...
                        previous_body = None
                    elif page_number == 1 and rect[1] < first_heading_y:
                    ...
                    elif _looks_like_equation(block, body_font):
                        role = "equation"
                        confidence = 0.72
                        label_match = re.search(r"\((\d+[a-z]?)\)\s*$", text)
                        label = f"Equation {label_match.group(1)}" if label_match else None
                        visuals.append(
                            {
                                "objectId": f"equation-p{page_number}-{block_id.rsplit('b', 1)[-1]}",
                                "kind": "equation",
                                "label": label,
                                "bboxNormalized": [round(value, 5) for value in rect],
                                "captionBlockIds": [],
                                "insertAfterBlockId": last_visible_block_id,
                                "confidence": confidence,
                                "warnings": ["Equation is preserved from the original PDF region."],
                            }
                        )
                    else:
...
def evaluate_structure(gold: dict[str, Any], candidate: dict[str, Any]) -> dict[str, Any]:
    ...
    gold_visuals = {
        str(value.get("label", "")).lower(): value
        for page in gold["pages"]
    ...
    shared_visuals = sorted(set(gold_visuals) & set(candidate_visuals))
    visual_ious = [
        _iou(gold_visuals[label]["bboxNormalized"], candidate_visuals[label]["bboxNormalized"])
        for label in shared_visuals
    ]
    return {
        "blocks": {
        ...
        "visuals": {
            ...
            "matched": len(shared_visuals),
            "meanIoU": sum(visual_ious) / len(visual_ious) if visual_ious else 0.0,
            "perLabelIoU": dict(zip(shared_visuals, visual_ious, strict=True)),
        },
    }
